...
@Component({
    ...
    template: `
        <app-page-layout [title]="title" [appName]="appName" [stepNumber]="'42'" [bgColor]="bgColor">
            <div rightContent class="flex flex-col items-center gap-4 w-full h-full">
                <p class="text-xl text-gray-700">The page you're looking for doesn't exist.</p>

                <div class="flex gap-4">
                    <app-button variant="secondary" fontWeight="semibold" (onClick)="goBack()"> Go Back </app-button>

                    <app-button variant="primary" fontWeight="semibold" (onClick)="showListDialog = true"> Show List </app-button>
                </div>
            </div>
        </app-page-layout>
